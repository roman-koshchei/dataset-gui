<script lang="ts">
    ...
    function isSelectedLabelIndexValid() {
        ...
        return (
            selectedLabelIndex >= 0 && selectedLabelIndex < item.labels.length
        );
    }
    ...
    function handleMouseMove(e: MouseEvent) {
        ...
        if (mouseAction.type === "dragging") {
            ...
            if (mouseAction.handle === "tl") {
                ...
                label.width =
                    dragStartLabel.left + dragStartLabel.width - newLeft;
                label.height =
                    dragStartLabel.top + dragStartLabel.height - newTop;
                label.left = newLeft;
                label.top = newTop;
                ...
                label.width = Math.max(
                    0.01,
                    Math.min(
                        1 - dragStartLabel.left,
                        dragStartLabel.width + deltaX,
                    ),
                );
                label.height =
                    dragStartLabel.top + dragStartLabel.height - newTop;
                ...
                label.height = Math.max(
                    0.01,
                    Math.min(
                        1 - dragStartLabel.top,
                        dragStartLabel.height + deltaY,
                    ),
                );
                label.left = newLeft;
